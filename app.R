...
data <- data %>%
  relocate(sector_productivo, area_geografica, sexo, rango_de_edad, 
           nivel_educativo, nivel_educativo_2, quintil, tramo_de_edad, 
           nivel_de_ingreso, escala_de_sueldos, decil, .after = everything()) %>%
  mutate(p2 = case_when(p2 == "Crecimento" ~ "Crecimiento",
                        TRUE ~ p2)) %>%
  mutate(
    nomindicador = case_when(is.na(nomindicador) ~ "-",
                             TRUE ~ nomindicador),
    definicion = case_when(is.na(definicion) ~ "-",
                           TRUE ~ definicion),
    metodo_de_agregacion = case_when(is.na(metodo_de_agregacion) ~ "-",
                                     TRUE ~ metodo_de_agregacion),
    concepto_estadistico_y_metodologia = case_when(
      is.na(concepto_estadistico_y_metodologia) ~ "-",
      TRUE ~ concepto_estadistico_y_metodologia
    ),
    limitaciones_y_excepciones = case_when(
      is.na(limitaciones_y_excepciones) ~ "-",
...
vars_corte <- data %>%
  filter(!is.na(sector_productivo) | !is.na(area_geografica) | !is.na(sexo) | !is.na(rango_de_edad) | !is.na(nivel_educativo) | !is.na(nivel_educativo_2) | !is.na(quintil) | !is.na(tramo_de_edad) | !is.na(nivel_de_ingreso) | !is.na(escala_de_sueldos) | !is.na(decil)) %>%
      ...
      tabPanel("Emisiones",
               ...
               fluidRow(
                 ...
                 mainPanel(
                   ...
                   conditionalPanel(condition = "input.visualizador_EN_emisiones == 'Anual mapa'",
                                    withSpinner(
                                      leafletOutput("map_EN_emisiones"),
                                      type = 2
                                    )),
  ...
  tabPanel(
    ...
    fluidRow(
      sidebarPanel(
        ...
        selectInput(
          inputId = "indicador_SA_sa",
          label = "Indicador",
          choices = data %>%
            filter(str_detect(p1, "Sostenibilidad ambiental")) %>%
            distinct(nomindicador) %>%
            arrange(nomindicador) %>%
            pull(nomindicador)
        ),
        
        uiOutput("corte_SA_sa"),
        
        uiOutput("fecha_dat_SA_sa"),
        
        br(),
        
        conditionalPanel(condition = "input.visualizador_SA_sa != 'Anual mapa'",
                         uiOutput("sel_SA_sa_pais")),
        
        br(),
        
        uiOutput("sel_SA_sa_region"),
      ...
      mainPanel(
        ...
        tags$h5(uiOutput("subtitle_dat_SA_sa")),
        
        br(),
        
        conditionalPanel(condition = "input.visualizador_SA_sa != 'Anual mapa'",
                         withSpinner(
                           plotOutput("p_dat_SA_sa", height = "600px"),
                           type = 2
                         )),
        
        conditionalPanel(condition = "input.visualizador_SA_sa == 'Anual mapa'",
                         withSpinner(leafletOutput("map_SA_sa"),
                                     type = 2)),
        
        conditionalPanel(
          condition = "input.visualizador_SA_sa != 'Anual mapa'",
          downloadButton(outputId = "baja_plot_SA_sa",
                         label = "Descarga el gráfico")
        ),
        br(),
        br(),
        withSpinner(DTOutput("tab_dat_SA_sa"),
                    type = 2),
        br(),
        downloadButton("dl_tabla_dat_SA_sa", "Descarga la tabla"),
        br(),
        br()
      ...
      tabPanel("Natalidad",
               ...
               fluidRow(
                 sidebarPanel(
                   ...
                   selectInput(
                     inputId = "indicador_DEMO_nat",
                     label = "Indicador",
                     choices = data %>%
                       filter(str_detect(p2, "Natalidad")) %>%
                       distinct(nomindicador) %>%
                       arrange(nomindicador) %>%
                       pull(nomindicador)
                   ),

                   uiOutput("corte_DEMO_nat"),

                   uiOutput("fecha_dat_DEMO_nat"),

                   br(),

                   conditionalPanel(condition = "input.visualizador_DEMO_nat != 'Anual mapa'",
                                    uiOutput("sel_DEMO_nat_pais")),

                   br(),

                   uiOutput("sel_DEMO_nat_region"),
                 ...
                 mainPanel(
                   ...
                   conditionalPanel(condition = "input.visualizador_DEMO_nat != 'Anual mapa'",
                                    withSpinner(
                                      plotOutput("p_dat_DEMO_nat", height = "600px"),
                                      type = 2
                                    )),

                   conditionalPanel(condition = "input.visualizador_DEMO_nat == 'Anual mapa'",
                                    withSpinner(leafletOutput("map_DEMO_nat"),
                                                type = 2)),

                   conditionalPanel(
                     condition = "input.visualizador_DEMO_nat != 'Anual mapa'",
                     downloadButton(outputId = "baja_plot_DEMO_nat",
                                    label = "Descarga el gráfico")
                   ),
                   br(),
                   br(),
                   withSpinner(DTOutput("tab_dat_DEMO_nat"),
                               type = 2),
                   br(),
                   downloadButton("dl_tabla_dat_DEMO_nat", "Descarga la tabla"),
                   br(),
                   br()
                 )
               )),
      ...
      tabPanel("Mortalidad",
               ...
               fluidRow(
                 sidebarPanel(
                   ...
                   selectInput(
                     inputId = "visualizador_DEMO_morta",
                     label = "Visualización",
                     choices = c("Serie de tiempo",
                                 "Anual gráfico",
                                 "Anual mapa"),
                   ...
                   selectInput(
                     inputId = "indicador_DEMO_morta",
                     label = "Indicador",
                     choices = data %>%
                       filter(str_detect(p2, "Mortalidad")) %>%
                       distinct(nomindicador) %>%
                       arrange(nomindicador) %>%
                       pull(nomindicador)
                   ),

                   uiOutput("corte_DEMO_morta"),

                   uiOutput("fecha_dat_DEMO_morta"),

                   br(),

                   conditionalPanel(condition = "input.visualizador_DEMO_morta != 'Anual mapa'",
                                    uiOutput("sel_DEMO_morta_pais")),

                   br(),

                   uiOutput("sel_DEMO_morta_region"),
                 ...
                 mainPanel(
                   ...
                   tags$h5(uiOutput("subtitle_dat_DEMO_morta")),

                   br(),

                   conditionalPanel(condition = "input.visualizador_DEMO_morta != 'Anual mapa'",
                                    withSpinner(
                                      plotOutput("p_dat_DEMO_morta", height = "600px"),
                                      type = 2
                                    )),

                   conditionalPanel(condition = "input.visualizador_DEMO_morta == 'Anual mapa'",
                                    withSpinner(
                                      leafletOutput("map_DEMO_morta"),
                                      type = 2
                                    )),

                   conditionalPanel(
                     condition = "input.visualizador_DEMO_morta != 'Anual mapa'",
                     downloadButton(outputId = "baja_plot_DEMO_morta",
                                    label = "Descarga el gráfico")
                   ),
                   br(),
                   br(),
                   withSpinner(DTOutput("tab_dat_DEMO_morta"),
                               type = 2),
                   br(),
                   downloadButton("dl_tabla_dat_DEMO_morta", "Descarga la tabla"),
                   br(),
                   br()
                 )
               )),
      ...
      tabPanel("Fertilidad",
               ...
               fluidRow(
                 sidebarPanel(
                   ...
                   selectInput(
                     inputId = "visualizador_DEMO_fert",
                     label = "Visualización",
                     choices = c("Serie de tiempo",
                                 "Anual gráfico",
                                 "Anual mapa"),
                   ...
                   selectInput(
                     inputId = "indicador_DEMO_fert",
                     label = "Indicador",
                     choices = data %>%
                       filter(str_detect(p2, "Fertilidad")) %>%
                       distinct(nomindicador) %>%
                       arrange(nomindicador) %>%
                       pull(nomindicador)
                   ),

                   uiOutput("corte_DEMO_fert"),

                   uiOutput("fecha_dat_DEMO_fert"),

                   br(),

                   conditionalPanel(condition = "input.visualizador_DEMO_fert != 'Anual mapa'",
                                    uiOutput("sel_DEMO_fert_pais")),

                   br(),

                   uiOutput("sel_DEMO_fert_region"),
                 ...
                 mainPanel(
                   ...
                   conditionalPanel(condition = "input.visualizador_DEMO_fert != 'Anual mapa'",
                                    withSpinner(
                                      plotOutput("p_dat_DEMO_fert", height = "600px"),
                                      type = 2
                                    )),

                   conditionalPanel(condition = "input.visualizador_DEMO_fert == 'Anual mapa'",
                                    withSpinner(
                                      leafletOutput("map_DEMO_fert"),
                                      type = 2
                                    )),

                   conditionalPanel(
                     condition = "input.visualizador_DEMO_fert != 'Anual mapa'",
                     downloadButton(outputId = "baja_plot_DEMO_fert",
                                    label = "Descarga el gráfico")
                   ),
                   br(),
                   br(),
                   withSpinner(DTOutput("tab_dat_DEMO_fert"),
                               type = 2),
                   br(),
                   downloadButton("dl_tabla_dat_DEMO_fert", "Descarga la tabla"),
                   br(),
                   br()
                 )
               )),
      ...
      tabPanel("Población",
               ...
               fluidRow(
                 sidebarPanel(
                   ...
                   selectInput(
                     inputId = "indicador_DEMO_pob",
                     label = "Indicador",
                     choices = data %>%
                       filter(str_detect(p2, "Población")) %>%
                       distinct(nomindicador) %>%
                       arrange(nomindicador) %>%
                       pull(nomindicador)
                   ),

                   uiOutput("corte_DEMO_pob"),

                   uiOutput("fecha_dat_DEMO_pob"),

                   br(),

                   conditionalPanel(condition = "input.visualizador_DEMO_pob != 'Anual mapa'",
                                    uiOutput("sel_DEMO_pob_pais")),

                   br(),

                   uiOutput("sel_DEMO_pob_region"),
                 ...
                 mainPanel(
                   ...
                   conditionalPanel(condition = "input.visualizador_DEMO_pob != 'Anual mapa'",
                                    withSpinner(
                                      plotOutput("p_dat_DEMO_pob", height = "600px"),
                                      type = 2
                                    )),

                   conditionalPanel(condition = "input.visualizador_DEMO_pob == 'Anual mapa'",
                                    withSpinner(leafletOutput("map_DEMO_pob"),
                                                type = 2)),

                   conditionalPanel(
                     condition = "input.visualizador_DEMO_pob != 'Anual mapa'",
                     downloadButton(outputId = "baja_plot_DEMO_pob",
                                    label = "Descarga el gráfico")
                   ),
                   br(),
                   br(),
                   withSpinner(DTOutput("tab_dat_DEMO_pob"),
                               type = 2),
                   br(),
                   downloadButton("dl_tabla_dat_DEMO_pob", "Descarga la tabla"),
                   br(),
                   br()
                 )
               ))
    ...
    server <- function(session, input, output) {
      ...
      output$corte_CP_comp <- renderUI({
        if (input$indicador_CP_comp %in% vars_corte) {
          selectInput(
            inputId = "corte_CP_comp",
            label = "Seleccione categorías",
            choices =  dat_CP_comp() %>%
              distinct(get(names(
                dat_CP_comp()[, ncol(dat_CP_comp())]
              ))) %>%
              pull(),
      ...
      output$p_dat_CP_comp <- renderPlot({
        if (input$visualizador_CP_comp == "Serie de tiempo") {
          ...
          plot_CP_comp <- ggplot(data = dat_CP_comp_simple(),
                                 aes(x = fecha, y = valor)) +
      ...
      output$map_CP_comp <- renderLeaflet({
        ...
        leaflet(world_spdf) %>%
          ...
          addLegend(
            ...
            opacity = 0.9,
            title = wrapit(input$indicador_CP_comp, 40),
            position = "bottomleft"
          )
      ...
      output$corte_CP_precios <- renderUI({
        if (input$indicador_CP_precios %in% vars_corte) {
          selectInput(
            inputId = "corte_CP_precios",
            label = "Seleccione categorías",
            choices =  dat_CP_precios() %>%
              distinct(get(names(
                dat_CP_precios()[, ncol(dat_CP_precios())]
              ))) %>%
              pull(),
      ...
      dat_CP_precios_simple <- reactive({
        if (input$indicador_CP_precios %in% vars_corte) {
          dat_CP_precios() %>%
            ...
            filter(
              cod_pais %in% input$chbox_pais_CP_precios |
      ...
      output$p_dat_CP_precios <- renderPlot({
        if (input$visualizador_CP_precios == "Serie de tiempo") {
          req(input$fecha_dat_CP_precios,
              input$indicador_CP_precios)
          
          plot_CP_precios <- ggplot(data = dat_CP_precios_simple(),
                                    aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
      ...
      output$map_CP_precios <- renderLeaflet({
        ...
        world_spdf@data <- world_spdf@data %>%
          left_join(
            select(dat_CP_precios_anual(), pais_eng, valor_original),
            by = c("NAME" = "pais_eng")
          )
      ...
      output$corte_DE_crec <- renderUI({
        if (input$indicador_DE_crec %in% vars_corte) {
          selectInput(
            inputId = "corte_DE_crec",
            label = "Seleccione categorías",
            choices =  dat_DE_crec() %>%
              distinct(get(names(
                dat_DE_crec()[, ncol(dat_DE_crec())]
              ))) %>%
              pull(),
      ...
      output$p_dat_DE_crec <- renderPlot({
        if (input$visualizador_DE_crec == "Serie de tiempo") {
          ...
          plot_DE_crec <- ggplot(data = dat_DE_crec_simple(),
                                 aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
      ...
      output$corte_DE_prod <- renderUI({
        if (input$indicador_DE_prod %in% vars_corte) {
          selectInput(
            inputId = "corte_DE_prod",
            label = "Seleccione categorías",
            choices =  dat_DE_prod() %>%
              distinct(get(names(
                dat_DE_prod()[, ncol(dat_DE_prod())]
              ))) %>%
              pull(),
      ...
      output$p_dat_DE_prod <- renderPlot({
        if (input$visualizador_DE_prod == "Serie de tiempo") {
          ...
          plot_DE_prod <- ggplot(data = dat_DE_prod_simple(),
                                 aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
      ...
      output$corte_DE_estr <- renderUI({
        if (input$indicador_DE_estr %in% vars_corte) {
          selectInput(
            inputId = "corte_DE_estr",
            label = "Seleccione categorías",
            choices =  dat_DE_estr() %>%
              distinct(get(names(
                dat_DE_estr()[, ncol(dat_DE_estr())]
              ))) %>%
              pull(),
      ...
      output$p_dat_DE_estr <- renderPlot({
        if (input$visualizador_DE_estr == "Serie de tiempo") {
          ...
          plot_DE_estr <- ggplot(data = dat_DE_estr_simple(),
                                 aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
      ...
      output$corte_DE_inv <- renderUI({
        if (input$indicador_DE_inv %in% vars_corte) {
          selectInput(
            inputId = "corte_DE_inv",
            label = "Seleccione categorías",
            choices =  dat_DE_inv() %>%
              distinct(get(names(
                dat_DE_inv()[, ncol(dat_DE_inv())]
              ))) %>%
              pull(),
            selected = dat_DE_inv() %>%
              filter(jerarquia == 1) %>%
      ...
      output$p_dat_DE_inv <- renderPlot({
        if (input$visualizador_DE_inv == "Serie de tiempo") {
          ...
          plot_DE_inv <- ggplot(data = dat_DE_inv_simple(),
                                aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
      ...
      output$corte_DE_cti <- renderUI({
        if (input$indicador_DE_cti %in% vars_corte) {
          selectInput(
            inputId = "corte_DE_cti",
            label = "Seleccione categorías",
            choices =  dat_DE_cti() %>%
              distinct(get(names(
                dat_DE_cti()[, ncol(dat_DE_cti())]
              ))) %>%
              pull(),
            selected = dat_DE_cti() %>%
              filter(jerarquia == 1) %>%
      ...
      output$p_dat_DE_cti <- renderPlot({
        if (input$visualizador_DE_cti == "Serie de tiempo") {
          ...
          plot_DE_cti <- ggplot(data = dat_DE_cti_simple(),
                                aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
      ...
      output$corte_DE_infra <- renderUI({
        if (input$indicador_DE_infra %in% vars_corte) {
          selectInput(
            inputId = "corte_DE_infra",
            label = "Seleccione categorías",
            choices =  dat_DE_infra() %>%
              distinct(get(names(
                dat_DE_infra()[, ncol(dat_DE_infra())]
              ))) %>%
              pull(),
      ...
      output$p_dat_DE_infra <- renderPlot({
        if (input$visualizador_DE_infra == "Serie de tiempo") {
          ...
          plot_DE_infra <- ggplot(data = dat_DE_infra_simple(),
                                  aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
            ...
            if (input$indicador_DE_infra %in% vars_corte) {
              ggtitle(paste0(
                input$indicador_DE_infra,
                " (",
                input$corte_DE_infra,
                ")"
              ))
      ...
      output$corte_SF_ahorro <- renderUI({
        if (input$indicador_SF_ahorro %in% vars_corte) {
          selectInput(
            inputId = "corte_SF_ahorro",
            label = "Seleccione categorías",
            choices =  dat_SF_ahorro() %>%
              distinct(get(names(
                dat_SF_ahorro()[, ncol(dat_SF_ahorro())]
              ))) %>%
              pull(),
      ...
      output$p_dat_SF_ahorro <- renderPlot({
        if (input$visualizador_SF_ahorro == "Serie de tiempo") {
          ...
          plot_SF_ahorro <- ggplot(data = dat_SF_ahorro_simple(),
                                   aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
            ...
            if (input$indicador_SF_ahorro %in% vars_corte) {
              ggtitle(paste0(
                input$indicador_SF_ahorro,
                " (",
                input$corte_SF_ahorro,
                ")"
              ))
      ...
      output$map_SF_ahorro <- renderLeaflet({
        ...
        world_spdf@data <- world_spdf@data %>%
          left_join(
            select(dat_SF_ahorro_anual(), pais_eng, valor_original),
            by = c("NAME" = "pais_eng")
          )
      ...
      output$corte_SF_externa <- renderUI({
        if (input$indicador_SF_externa %in% vars_corte) {
          selectInput(
            inputId = "corte_SF_externa",
            label = "Seleccione categorías",
            choices =  dat_SF_externa() %>%
              distinct(get(names(
                dat_SF_externa()[, ncol(dat_SF_externa())]
              ))) %>%
              pull(),
      ...
      dat_SF_externa_simple <- reactive({
        if (input$indicador_SF_externa %in% vars_corte) {
          dat_SF_externa() %>%
            ...
            filter(
              cod_pais %in% input$chbox_pais_SF_externa |
      ...
      output$p_dat_SF_externa <- renderPlot({
        if (input$visualizador_SF_externa == "Serie de tiempo") {
          req(input$fecha_dat_SF_externa,
              input$indicador_SF_externa)
          
          plot_SF_externa <- ggplot(data = dat_SF_externa_simple(),
                                    aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
      ...
      output$map_SF_externa <- renderLeaflet({
        ...
        world_spdf@data <- world_spdf@data %>%
          left_join(
            select(dat_SF_externa_anual(), pais_eng, valor_original),
            by = c("NAME" = "pais_eng")
          )
      ...
      output$corte_SF_inv <- renderUI({
        if (input$indicador_SF_inv %in% vars_corte) {
          selectInput(
            inputId = "corte_SF_inv",
            label = "Seleccione categorías",
            choices =  dat_SF_inv() %>%
              distinct(get(names(
                dat_SF_inv()[, ncol(dat_SF_inv())]
              ))) %>%
              pull(),
            selected = dat_SF_inv() %>%
              filter(jerarquia == 1) %>%
      ...
      output$p_dat_SF_inv <- renderPlot({
        if (input$visualizador_SF_inv == "Serie de tiempo") {
          ...
          plot_SF_inv <- ggplot(data = dat_SF_inv_simple(),
                                aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
      ...
      output$corte_SF_mon <- renderUI({
        if (input$indicador_SF_mon %in% vars_corte) {
          selectInput(
            inputId = "corte_SF_mon",
            label = "Seleccione categorías",
            choices =  dat_SF_mon() %>%
              distinct(get(names(
                dat_SF_mon()[, ncol(dat_SF_mon())]
              ))) %>%
              pull(),
            selected = dat_SF_mon() %>%
              filter(jerarquia == 1) %>%
      ...
      output$p_dat_SF_mon <- renderPlot({
        if (input$visualizador_SF_mon == "Serie de tiempo") {
          ...
          plot_SF_mon <- ggplot(data = dat_SF_mon_simple(),
                                aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
      ...
      output$corte_EN_consumo <- renderUI({
        if (input$indicador_EN_consumo %in% vars_corte) {
          selectInput(
            inputId = "corte_EN_consumo",
            label = "Seleccione categorías",
            choices =  dat_EN_consumo() %>%
              distinct(get(names(
                dat_EN_consumo()[, ncol(dat_EN_consumo())]
              ))) %>%
              pull(),
      ...
      dat_EN_consumo_simple <- reactive({
        if (input$indicador_EN_consumo %in% vars_corte) {
          dat_EN_consumo() %>%
            ...
            filter(
              cod_pais %in% input$chbox_pais_EN_consumo |
      ...
      output$p_dat_EN_consumo <- renderPlot({
        if (input$visualizador_EN_consumo == "Serie de tiempo") {
          req(input$fecha_dat_EN_consumo,
              input$indicador_EN_consumo)
          
          plot_EN_consumo <- ggplot(data = dat_EN_consumo_simple(),
                                    aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
      ...
      output$map_EN_consumo <- renderLeaflet({
        ...
        world_spdf@data <- world_spdf@data %>%
          left_join(
            select(dat_EN_consumo_anual(), pais_eng, valor_original),
            by = c("NAME" = "pais_eng")
          )
      ...
      output$rel_dat_EN_emisiones <- renderUI({
        helpText(HTML(paste(
          "<b>Relvancia:</b>",
          unique(dat_EN_emisiones()$relevancia)
        )))
      })
      ...
      output$corte_EN_emisiones <- renderUI({
        if (input$indicador_EN_emisiones %in% vars_corte) {
          selectInput(
            inputId = "corte_EN_emisiones",
            label = "Seleccione categorías",
            choices =  dat_EN_emisiones() %>%
              distinct(get(names(
                dat_EN_emisiones()[, ncol(dat_EN_emisiones())]
              ))) %>%
              pull(),
      ...
      dat_EN_emisiones_simple <- reactive({
        if (input$indicador_EN_emisiones %in% vars_corte) {
          dat_EN_emisiones() %>%
            filter(get(names(dat_EN_emisiones()[, ncol(dat_EN_emisiones())])) %in% input$corte_EN_emisiones) %>%
            filter(
              fecha >= input$fecha_dat_EN_emisiones[1] &
                fecha <= input$fecha_dat_EN_emisiones[2]
            ) %>%
            filter(
              cod_pais %in% input$chbox_pais_EN_emisiones |
      ...
      output$p_dat_EN_emisiones <- renderPlot({
        if (input$visualizador_EN_emisiones == "Serie de tiempo") {
          req(input$fecha_dat_EN_emisiones,
              input$indicador_EN_emisiones)
          
          plot_EN_emisiones <- ggplot(data = dat_EN_emisiones_simple(),
                                      aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
      ...
      output$map_EN_emisiones <- renderLeaflet({
        ...
        world_spdf@data <- world_spdf@data %>%
          left_join(
            select(dat_EN_emisiones_anual(), pais_eng, valor_original),
            by = c("NAME" = "pais_eng")
          )
      ...
      output$corte_EN_prod <- renderUI({
        if (input$indicador_EN_prod %in% vars_corte) {
          selectInput(
            inputId = "corte_EN_prod",
            label = "Seleccione categorías",
            choices =  dat_EN_prod() %>%
              distinct(get(names(
                dat_EN_prod()[, ncol(dat_EN_prod())]
              ))) %>%
              pull(),
      ...
      output$p_dat_EN_prod <- renderPlot({
        if (input$visualizador_EN_prod == "Serie de tiempo") {
          ...
          plot_EN_prod <- ggplot(data = dat_EN_prod_simple(),
                                 aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
      ...
      # Titulo
      output$title_dat_SA_sa <- renderUI({
        helpText(HTML(unique(dat_SA_sa()$nomindicador)))
      })
      
      # Subtitulo
      output$subtitle_dat_SA_sa <- renderUI({
        helpText(HTML(unique(dat_SA_sa()$definicion)))
      })
      
      # Metodología
      output$info_dat_SA_sa <- renderUI({
        helpText(HTML(
          paste(
            "<b>Método de Agregación:</b>",
            unique(dat_SA_sa()$metodo_de_agregacion),
            "<b>Metodología:</b>",
            unique(dat_SA_sa()$concepto_estadistico_y_metodologia)
          )
        ))
      })
      
      # Relevancia:
      output$rel_dat_SA_sa <- renderUI({
        helpText(HTML(paste(
          "<b>Relvancia:</b>", unique(dat_SA_sa()$relevancia)
        )))
      })
      
      
      output$fecha_dat_SA_sa <- renderUI({
        if (input$visualizador_SA_sa == "Serie de tiempo") {
          tagList(
            ...
            div(
              ...
              sliderTextInput(
                inputId = "fecha_dat_SA_sa",
                label = "Seleccione años",
                choices = sort(unique(dat_SA_sa()$fecha)),
                selected = c(min(dat_SA_sa()$fecha),
                             max(dat_SA_sa()$fecha)),
                hide_min_max = T,
                grid = TRUE,
                width = "100%"
      ...
      output$corte_SA_sa <- renderUI({
        if (input$indicador_SA_sa %in% vars_corte) {
          selectInput(
            inputId = "corte_SA_sa",
            label = "Seleccione categorías",
            choices =  dat_SA_sa() %>%
              distinct(get(names(dat_SA_sa(
              )[, ncol(dat_SA_sa())]))) %>%
              pull(),
            selected = dat_SA_sa() %>%
              filter(jerarquia == 1) %>%
      ...
      # Checkbox por pais
      output$sel_SA_sa_pais <- renderUI({
        if (input$visualizador_SA_sa == "Serie de tiempo") {
          dropdown(
            label = "Seleccione países",
            status = "default",
      ...
      dat_SA_sa_anual <- reactive({
        if (input$indicador_SA_sa %in% vars_corte) {
          dat_SA_sa() %>%
            filter(get(names(dat_SA_sa()[, ncol(dat_SA_sa())])) %in% input$corte_SA_sa) %>%
            filter(fecha == input$fecha_SA_sa)
      ...
      dat_SA_sa_simple <- reactive({
        if (input$indicador_SA_sa %in% vars_corte) {
          dat_SA_sa() %>%
            filter(get(names(dat_SA_sa()[, ncol(dat_SA_sa())])) %in% input$corte_SA_sa) %>%
            filter(fecha >= input$fecha_dat_SA_sa[1] &
                     fecha <= input$fecha_dat_SA_sa[2]) %>%
            filter(cod_pais %in% input$chbox_pais_SA_sa |
                     pais %in% input$chbox_reg_SA_sa)
      ...
      # Gráficos SA_sa
      output$p_dat_SA_sa <- renderPlot({
        if (input$visualizador_SA_sa == "Serie de tiempo") {
          req(input$fecha_dat_SA_sa, input$indicador_SA_sa)
          
          plot_SA_sa <- ggplot(data = dat_SA_sa_simple(),
                               aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
            ...
            labs(x = "",
                 ...
                 caption = wrapit(
                   paste(
                     "Fuente: Unidad de Métodos y Acceso a Datos (FCS - UdelaR) en base a datos de",
                     unique(dat_SA_sa_simple()$fuente)
                   )
                 )) +
            scale_y_continuous(labels = addUnits) +
            scale_colour_manual(name = "", values = paleta_expandida) +
            if (input$indicador_SA_sa %in% vars_corte) {
              ggtitle(paste0(input$indicador_SA_sa, " (", input$corte_SA_sa, ")"))
              
            } else {
              ggtitle(paste(input$indicador_SA_sa))
              
            }
          
          print(plot_SA_sa)
          ggsave(
            "www/indicador SA_sa.png",
            width = 30,
            height = 20,
            units = "cm"
          )
          
        } else if (input$visualizador_SA_sa == "Anual gráfico") {
          base_plot_SA_sa <- dat_SA_sa_anual() %>%
            filter(pais_region %in% input$chbox_pais_reg_SA_sa)
          
          validate(need(
            nrow(base_plot_SA_sa) > 0,
            'No hay datos disponible para esta búsqueda'
          ))
          
          plot_SA_sa <- ggplot(base_plot_SA_sa,
                               aes(x = fct_reorder(pais, valor), y = valor)) +
            geom_segment(
              aes(
                x = fct_reorder(pais, valor),
      ...
      output$map_SA_sa <- renderLeaflet({
        # Read this shape file with the rgdal library.
        world_spdf <- readOGR(dsn = "data" ,
                              layer = "TM_WORLD_BORDERS_SIMPL-0.3",
                              verbose = FALSE)
        ...
        # Pegar datos
        world_spdf@data <- world_spdf@data %>%
          left_join(select(dat_SA_sa_anual(), pais_eng, valor_original),
                    by = c("NAME" = "pais_eng"))
        ...
        leaflet(world_spdf) %>%
          ...
          addLegend(
            ...
            values =  ~ valor_original,
            opacity = 0.9,
            title = wrapit(input$indicador_SA_sa),
            position = "bottomleft"
          )
      ...
      # Botón descarga grafico
      output$baja_plot_SA_sa <- downloadHandler(filename <- function() {
        paste("indicador SA_sa", "png", sep = ".")
      },
      
      content <- function(file) {
        file.copy("www/indicador SA_sa.png", file)
      },
      contentType = "www/indicador SA_sa")
      
      
      ## Data series temporal
      
      # Data para tabla y exportar
      dat_SA_sa_st <- reactive({
        if (input$indicador_SA_sa %in% vars_corte) {
          dat_SA_sa() %>%
            filter(fecha >= input$fecha_dat_SA_sa[1] &
                     fecha <= input$fecha_dat_SA_sa[2]) %>%
            filter(cod_pais %in% input$chbox_pais_SA_sa |
                     pais %in% input$chbox_reg_SA_sa) %>%
            select(pais, fecha, names(dat_SA_sa()[, ncol(dat_SA_sa())]), valor) %>%
            arrange(desc(fecha), fct_reorder(pais, -valor))
      ...
      # Lista para descarga
      list_dat_SA_sa_st <- reactive({
        list_dat_SA_sa <- list(
          "Data" = dat_SA_sa_st(),
          "Metadata" = dat_SA_sa_m(),
          "Data Completa" = dat_SA_sa_c()
        )
      })
      ...
      output$tab_dat_SA_sa <- renderDT({
        if (input$indicador_SA_sa %in% vars_corte) {
          DT::datatable(
            ...
            caption = htmltools::tags$caption(input$indicador_SA_sa,
                                              style = "color:black; font-size:110%;")
          ) %>%
            formatCurrency(4, '', mark = ",")
          
        } else if (input$visualizador_SA_sa == "Serie de tiempo") {
          DT::datatable(
            dat_SA_sa_st(),
            rownames = FALSE,
            colnames = c("País/Región", "Fecha", "Valor"),
        ...
        } else if (input$visualizador_SA_sa %in% c("Anual gráfico", "Anual mapa")) {
          DT::datatable(
            dat_SA_sa_a(),
            rownames = FALSE,
            colnames = c("País/Región", "Fecha", "Valor"),
      ...
      # Descarga tabla
      output$dl_tabla_dat_SA_sa <- downloadHandler(
        filename = function() {
          paste("resultados-", input$indicador_SA_sa, ".xlsx", sep = "")
        },
        content = function(file) {
          if (input$visualizador_SA_sa == "Serie de tiempo") {
            openxlsx::write.xlsx(list_dat_SA_sa_st(), file)
            
          } else if (input$visualizador_SA_sa %in% c("Anual gráfico", "Anual mapa")) {
            openxlsx::write.xlsx(list_dat_SA_sa_a(), file)
      ...
      ##  20.  DEMO_nat (dat_DEMO_nat)   ====================================
      
      # Data DEMO_nat
      
      dat_DEMO_nat <- reactive({
        req(input$indicador_DEMO_nat)
      ...
      # Titulo
      output$title_dat_DEMO_nat <- renderUI({
        helpText(HTML(unique(dat_DEMO_nat()$nomindicador)))
      })
      
      # Subtitulo
      output$subtitle_dat_DEMO_nat <- renderUI({
        helpText(HTML(unique(dat_DEMO_nat()$definicion)))
      })
      
      # Metodología
      output$info_dat_DEMO_nat <- renderUI({
        helpText(HTML(
          paste(
            "<b>Método de Agregación:</b>",
            unique(dat_DEMO_nat()$metodo_de_agregacion),
            "<b>Metodología:</b>",
            unique(dat_DEMO_nat()$concepto_estadistico_y_metodologia)
          )
        ))
      })
      
      # Relevancia:
      output$rel_dat_DEMO_nat <- renderUI({
        helpText(HTML(paste(
          "<b>Relvancia:</b>", unique(dat_DEMO_nat()$relevancia)
        )))
      })
      
      
      output$fecha_dat_DEMO_nat <- renderUI({
        if (input$visualizador_DEMO_nat == "Serie de tiempo") {
          tagList(
            ...
            div(
              ...
              sliderTextInput(
                inputId = "fecha_dat_DEMO_nat",
                label = "Seleccione años",
                choices = sort(unique(dat_DEMO_nat()$fecha)),
                selected = c(min(dat_DEMO_nat()$fecha),
                             max(dat_DEMO_nat()$fecha)),
                hide_min_max = T,
                grid = TRUE,
                width = "100%"
              )
      ...
      output$corte_DEMO_nat <- renderUI({
        if (input$indicador_DEMO_nat %in% vars_corte) {
          selectInput(
            inputId = "corte_DEMO_nat",
            label = "Seleccione categorías",
            choices =  dat_DEMO_nat() %>%
              distinct(get(names(dat_DEMO_nat(
              )[, ncol(dat_DEMO_nat())]))) %>%
              pull(),
            selected = dat_DEMO_nat() %>%
              filter(jerarquia == 1) %>%
              distinct(get(names(dat_DEMO_nat(
              )[, ncol(dat_DEMO_nat())]))) %>%
              pull()
      ...
      # Checkbox por pais
      output$sel_DEMO_nat_pais <- renderUI({
        if (input$visualizador_DEMO_nat == "Serie de tiempo") {
          dropdown(
            label = "Seleccione países",
            status = "default",
          ...
          checkboxGroupInput(
            inputId = "chbox_pais_reg_DEMO_nat",
            label = "Mostrar:",
            inline = FALSE,
            choices = c("Países", "Regiones"),
            selected = "Países"
      ...
      dat_DEMO_nat_anual <- reactive({
        if (input$indicador_DEMO_nat %in% vars_corte) {
          dat_DEMO_nat() %>%
            filter(get(names(dat_DEMO_nat()[, ncol(dat_DEMO_nat())])) %in% input$corte_DEMO_nat) %>%
            filter(fecha == input$fecha_DEMO_nat)
      ...
      dat_DEMO_nat_simple <- reactive({
        if (input$indicador_DEMO_nat %in% vars_corte) {
          dat_DEMO_nat() %>%
            filter(get(names(dat_DEMO_nat()[, ncol(dat_DEMO_nat())])) %in% input$corte_DEMO_nat) %>%
            filter(fecha >= input$fecha_dat_DEMO_nat[1] &
                     fecha <= input$fecha_dat_DEMO_nat[2]) %>%
            filter(cod_pais %in% input$chbox_pais_DEMO_nat |
                     pais %in% input$chbox_reg_DEMO_nat)
      ...
      # Gráficos DEMO_nat
      output$p_dat_DEMO_nat <- renderPlot({
        if (input$visualizador_DEMO_nat == "Serie de tiempo") {
          req(input$fecha_dat_DEMO_nat, input$indicador_DEMO_nat)
          
          plot_DEMO_nat <- ggplot(data = dat_DEMO_nat_simple(),
                               aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
            ...
            labs(x = "",
                 ...
                 caption = wrapit(
                   paste(
                     "Fuente: Unidad de Métodos y Acceso a Datos (FCS - UdelaR) en base a datos de",
                     unique(dat_DEMO_nat_simple()$fuente)
                   )
                 )) +
            scale_y_continuous(labels = addUnits) +
            scale_colour_manual(name = "", values = paleta_expandida) +
            if (input$indicador_DEMO_nat %in% vars_corte) {
              ggtitle(paste0(input$indicador_DEMO_nat, " (", input$corte_DEMO_nat, ")"))
              
            } else {
              ggtitle(paste(input$indicador_DEMO_nat))
              
            }
          
          print(plot_DEMO_nat)
          ggsave(
            "www/indicador DEMO_nat.png",
            width = 30,
            height = 20,
            units = "cm"
          )
          
        } else if (input$visualizador_DEMO_nat == "Anual gráfico") {
          base_plot_DEMO_nat <- dat_DEMO_nat_anual() %>%
            filter(pais_region %in% input$chbox_pais_reg_DEMO_nat)
          
          validate(need(
            nrow(base_plot_DEMO_nat) > 0,
            'No hay datos disponible para esta búsqueda'
          ))
          
          plot_DEMO_nat <- ggplot(base_plot_DEMO_nat,
                               aes(x = fct_reorder(pais, valor), y = valor)) +
            geom_segment(
              aes(
                x = fct_reorder(pais, valor),
            ...
            labs(x = "",
                 ...
                 caption = wrapit(
                   paste(
                     "Fuente: Unidad de Métodos y Acceso a Datos (FCS - UdelaR) en base a datos de",
                     unique(dat_DEMO_nat_anual()$fuente)
                   )
                 )) +
            scale_y_continuous(labels = addUnits) +
            if (input$indicador_DEMO_nat %in% vars_corte) {
      ...
      output$map_DEMO_nat <- renderLeaflet({
        # Read this shape file with the rgdal library.
        world_spdf <- readOGR(dsn = "data" ,
                              layer = "TM_WORLD_BORDERS_SIMPL-0.3",
                              verbose = FALSE)
        ...
        # Pegar datos
        world_spdf@data <- world_spdf@data %>%
          left_join(select(dat_DEMO_nat_anual(), pais_eng, valor_original),
                    by = c("NAME" = "pais_eng"))
        ...
        leaflet(world_spdf) %>%
          ...
          addLegend(
            ...
            values =  ~ valor_original,
            opacity = 0.9,
            title = wrapit(input$indicador_DEMO_nat),
            position = "bottomleft"
          )
      ...
      # Botón descarga grafico
      output$baja_plot_DEMO_nat <- downloadHandler(filename <- function() {
        paste("indicador DEMO_nat", "png", sep = ".")
      },
      
      content <- function(file) {
        file.copy("www/indicador DEMO_nat.png", file)
      },
      contentType = "www/indicador DEMO_nat")
      
      
      ## Data series temporal
      
      # Data para tabla y exportar
      dat_DEMO_nat_st <- reactive({
        if (input$indicador_DEMO_nat %in% vars_corte) {
          dat_DEMO_nat() %>%
            filter(fecha >= input$fecha_dat_DEMO_nat[1] &
                     fecha <= input$fecha_dat_DEMO_nat[2]) %>%
            filter(cod_pais %in% input$chbox_pais_DEMO_nat |
                     pais %in% input$chbox_reg_DEMO_nat) %>%
            select(pais, fecha, names(dat_DEMO_nat()[, ncol(dat_DEMO_nat())]), valor) %>%
            arrange(desc(fecha), fct_reorder(pais, -valor))
      ...
      # Lista para descarga
      list_dat_DEMO_nat_st <- reactive({
        list_dat_DEMO_nat <- list(
          "Data" = dat_DEMO_nat_st(),
          "Metadata" = dat_DEMO_nat_m(),
          "Data Completa" = dat_DEMO_nat_c()
        )
      })
      ...
      # Tablas en shiny
      output$tab_dat_DEMO_nat <- renderDT({
        if (input$indicador_DEMO_nat %in% vars_corte) {
          DT::datatable(
            dat_DEMO_nat_st(),
            rownames = FALSE,
            ...
            caption = htmltools::tags$caption(input$indicador_DEMO_nat,
                                              style = "color:black; font-size:110%;")
          ) %>%
            formatCurrency(4, '', mark = ",")
          
        } else if (input$visualizador_DEMO_nat == "Serie de tiempo") {
          DT::datatable(
            dat_DEMO_nat_st(),
            rownames = FALSE,
            colnames = c("País/Región", "Fecha", "Valor"),
        ...
        } else if (input$visualizador_DEMO_nat %in% c("Anual gráfico", "Anual mapa")) {
          DT::datatable(
            dat_DEMO_nat_a(),
            rownames = FALSE,
            colnames = c("País/Región", "Fecha", "Valor"),
      ...
      # Descarga tabla
      output$dl_tabla_dat_DEMO_nat <- downloadHandler(
        filename = function() {
          paste("resultados-", input$indicador_DEMO_nat, ".xlsx", sep = "")
        },
        content = function(file) {
          if (input$visualizador_DEMO_nat == "Serie de tiempo") {
            openxlsx::write.xlsx(list_dat_DEMO_nat_st(), file)
            
          } else if (input$visualizador_DEMO_nat %in% c("Anual gráfico", "Anual mapa")) {
            openxlsx::write.xlsx(list_dat_DEMO_nat_a(), file)
            
          }
        }
      )
      
      ##  21.  DEMO_morta (dat_DEMO_morta)   ====================================
      
      # Data DEMO_morta
      
      dat_DEMO_morta <- reactive({
        req(input$indicador_DEMO_morta)
      ...
      # Titulo
      output$title_dat_DEMO_morta <- renderUI({
        helpText(HTML(unique(dat_DEMO_morta()$nomindicador)))
      })
      
      # Subtitulo
      output$subtitle_dat_DEMO_morta <- renderUI({
        helpText(HTML(unique(dat_DEMO_morta()$definicion)))
      })
      
      # Metodología
      output$info_dat_DEMO_morta <- renderUI({
        helpText(HTML(
          paste(
            "<b>Método de Agregación:</b>",
            unique(dat_DEMO_morta()$metodo_de_agregacion),
            "<b>Metodología:</b>",
            unique(dat_DEMO_morta()$concepto_estadistico_y_metodologia)
          )
        ))
      })
      
      # Relevancia:
      output$rel_dat_DEMO_morta <- renderUI({
        helpText(HTML(paste(
          "<b>Relvancia:</b>", unique(dat_DEMO_morta()$relevancia)
        )))
      })
      
      
      output$fecha_dat_DEMO_morta <- renderUI({
        if (input$visualizador_DEMO_morta == "Serie de tiempo") {
          tagList(
            ...
            div(
              ...
              sliderTextInput(
                inputId = "fecha_dat_DEMO_morta",
                label = "Seleccione años",
                choices = sort(unique(dat_DEMO_morta()$fecha)),
                selected = c(min(dat_DEMO_morta()$fecha),
                             max(dat_DEMO_morta()$fecha)),
                hide_min_max = T,
                grid = TRUE,
                width = "100%"
              )
      ...
      output$corte_DEMO_morta <- renderUI({
        if (input$indicador_DEMO_morta %in% vars_corte) {
          selectInput(
            inputId = "corte_DEMO_morta",
            label = "Seleccione categorías",
            choices =  dat_DEMO_morta() %>%
              distinct(get(names(dat_DEMO_morta(
              )[, ncol(dat_DEMO_morta())]))) %>%
              pull(),
            selected = dat_DEMO_morta() %>%
              filter(jerarquia == 1) %>%
              distinct(get(names(dat_DEMO_morta(
              )[, ncol(dat_DEMO_morta())]))) %>%
              pull()
      ...
      # Checkbox por pais
      output$sel_DEMO_morta_pais <- renderUI({
        if (input$visualizador_DEMO_morta == "Serie de tiempo") {
          dropdown(
            label = "Seleccione países",
            status = "default",
          ...
          checkboxGroupInput(
            inputId = "chbox_pais_reg_DEMO_morta",
            label = "Mostrar:",
            inline = FALSE,
            choices = c("Países", "Regiones"),
            selected = "Países"
      ...
      dat_DEMO_morta_anual <- reactive({
        if (input$indicador_DEMO_morta %in% vars_corte) {
          dat_DEMO_morta() %>%
            filter(get(names(dat_DEMO_morta()[, ncol(dat_DEMO_morta())])) %in% input$corte_DEMO_morta) %>%
            filter(fecha == input$fecha_DEMO_morta)
      ...
      dat_DEMO_morta_simple <- reactive({
        if (input$indicador_DEMO_morta %in% vars_corte) {
          dat_DEMO_morta() %>%
            filter(get(names(dat_DEMO_morta()[, ncol(dat_DEMO_morta())])) %in% input$corte_DEMO_morta) %>%
            filter(fecha >= input$fecha_dat_DEMO_morta[1] &
                     fecha <= input$fecha_dat_DEMO_morta[2]) %>%
            filter(cod_pais %in% input$chbox_pais_DEMO_morta |
                     pais %in% input$chbox_reg_DEMO_morta)
      ...
      # Gráficos DEMO_morta
      output$p_dat_DEMO_morta <- renderPlot({
        if (input$visualizador_DEMO_morta == "Serie de tiempo") {
          req(input$fecha_dat_DEMO_morta, input$indicador_DEMO_morta)
          
          plot_DEMO_morta <- ggplot(data = dat_DEMO_morta_simple(),
                                  aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
            ...
            labs(x = "",
                 ...
                 caption = wrapit(
                   paste(
                     "Fuente: Unidad de Métodos y Acceso a Datos (FCS - UdelaR) en base a datos de",
                     unique(dat_DEMO_morta_simple()$fuente)
                   )
                 )) +
            scale_y_continuous(labels = addUnits) +
            scale_colour_manual(name = "", values = paleta_expandida) +
            if (input$indicador_DEMO_morta %in% vars_corte) {
              ggtitle(paste0(input$indicador_DEMO_morta, " (", input$corte_DEMO_morta, ")"))
              
            } else {
              ggtitle(paste(input$indicador_DEMO_morta))
              
            }
          
          print(plot_DEMO_morta)
          ggsave(
            "www/indicador DEMO_morta.png",
            width = 30,
            height = 20,
            units = "cm"
          )
          
        } else if (input$visualizador_DEMO_morta == "Anual gráfico") {
          base_plot_DEMO_morta <- dat_DEMO_morta_anual() %>%
            filter(pais_region %in% input$chbox_pais_reg_DEMO_morta)
          
          validate(need(
            nrow(base_plot_DEMO_morta) > 0,
            'No hay datos disponible para esta búsqueda'
          ))
          
          plot_DEMO_morta <- ggplot(base_plot_DEMO_morta,
                                  aes(x = fct_reorder(pais, valor), y = valor)) +
            geom_segment(
              aes(
                x = fct_reorder(pais, valor),
            ...
            labs(x = "",
                 ...
                 caption = wrapit(
                   paste(
                     "Fuente: Unidad de Métodos y Acceso a Datos (FCS - UdelaR) en base a datos de",
                     unique(dat_DEMO_morta_anual()$fuente)
                   )
                 )) +
            scale_y_continuous(labels = addUnits) +
            if (input$indicador_DEMO_morta %in% vars_corte) {
      ...
      output$map_DEMO_morta <- renderLeaflet({
        # Read this shape file with the rgdal library.
        world_spdf <- readOGR(dsn = "data" ,
                              layer = "TM_WORLD_BORDERS_SIMPL-0.3",
                              verbose = FALSE)
        ...
        # Pegar datos
        world_spdf@data <- world_spdf@data %>%
          left_join(select(dat_DEMO_morta_anual(), pais_eng, valor_original),
                    by = c("NAME" = "pais_eng"))
        ...
        leaflet(world_spdf) %>%
          ...
          addLegend(
            ...
            values =  ~ valor_original,
            opacity = 0.9,
            title = wrapit(input$indicador_DEMO_morta),
            position = "bottomleft"
          )
      ...
      # Botón descarga grafico
      output$baja_plot_DEMO_morta <- downloadHandler(filename <- function() {
        paste("indicador DEMO_morta", "png", sep = ".")
      },
      
      content <- function(file) {
        file.copy("www/indicador DEMO_morta.png", file)
      },
      contentType = "www/indicador DEMO_morta")
      
      
      ## Data series temporal
      
      # Data para tabla y exportar
      dat_DEMO_morta_st <- reactive({
        if (input$indicador_DEMO_morta %in% vars_corte) {
          dat_DEMO_morta() %>%
            filter(fecha >= input$fecha_dat_DEMO_morta[1] &
                     fecha <= input$fecha_dat_DEMO_morta[2]) %>%
            filter(cod_pais %in% input$chbox_pais_DEMO_morta |
                     pais %in% input$chbox_reg_DEMO_morta) %>%
            select(pais, fecha, names(dat_DEMO_morta()[, ncol(dat_DEMO_morta())]), valor) %>%
            arrange(desc(fecha), fct_reorder(pais, -valor))
      ...
      # Lista para descarga
      list_dat_DEMO_morta_st <- reactive({
        list_dat_DEMO_morta <- list(
          "Data" = dat_DEMO_morta_st(),
          "Metadata" = dat_DEMO_morta_m(),
          "Data Completa" = dat_DEMO_morta_c()
        )
      })
      ...
      ## Data por año
      # Data para tabla y exportar
      dat_DEMO_morta_a <- reactive({
        if (input$indicador_DEMO_morta %in% vars_corte) {
          dat_DEMO_morta_anual() %>%
            filter(pais_region %in% input$chbox_pais_reg_DEMO_morta) %>%
            select(pais, fecha, names(dat_DEMO_morta()[, ncol(dat_DEMO_morta())]), valor) %>%
            arrange(desc(fecha), fct_reorder(pais, -valor))
      ...
      # Lista para descarga
      list_dat_DEMO_morta_a <- reactive({
        list_dat_DEMO_morta <- list(
          "Data" = dat_DEMO_morta_a(),
          "Metadata" = dat_DEMO_morta_m(),
          "Data Completa" = dat_DEMO_morta_c()
        )
      })
      ...
      # Tablas en shiny
      output$tab_dat_DEMO_morta <- renderDT({
        if (input$indicador_DEMO_morta %in% vars_corte) {
          DT::datatable(
            dat_DEMO_morta_st(),
            rownames = FALSE,
            ...
            caption = htmltools::tags$caption(input$indicador_DEMO_morta,
                                              style = "color:black; font-size:110%;")
          ) %>%
            formatCurrency(4, '', mark = ",")
          
        } else if (input$visualizador_DEMO_morta == "Serie de tiempo") {
          DT::datatable(
            dat_DEMO_morta_st(),
            rownames = FALSE,
            colnames = c("País/Región", "Fecha", "Valor"),
        ...
        } else if (input$visualizador_DEMO_morta %in% c("Anual gráfico", "Anual mapa")) {
          DT::datatable(
            dat_DEMO_morta_a(),
            rownames = FALSE,
            colnames = c("País/Región", "Fecha", "Valor"),
      ...
      # Descarga tabla
      output$dl_tabla_dat_DEMO_morta <- downloadHandler(
        filename = function() {
          paste("resultados-", input$indicador_DEMO_morta, ".xlsx", sep = "")
        },
        content = function(file) {
          if (input$visualizador_DEMO_morta == "Serie de tiempo") {
            openxlsx::write.xlsx(list_dat_DEMO_morta_st(), file)
            
          } else if (input$visualizador_DEMO_morta %in% c("Anual gráfico", "Anual mapa")) {
            openxlsx::write.xlsx(list_dat_DEMO_morta_a(), file)
      ...
      ##  22.  DEMO_fert (dat_DEMO_fert)   ====================================
      
      # Data DEMO_fert
      
      dat_DEMO_fert <- reactive({
        req(input$indicador_DEMO_fert)
      ...
      # Titulo
      output$title_dat_DEMO_fert <- renderUI({
        helpText(HTML(unique(dat_DEMO_fert()$nomindicador)))
      })
      
      # Subtitulo
      output$subtitle_dat_DEMO_fert <- renderUI({
        helpText(HTML(unique(dat_DEMO_fert()$definicion)))
      })
      
      # Metodología
      output$info_dat_DEMO_fert <- renderUI({
        helpText(HTML(
          paste(
            "<b>Método de Agregación:</b>",
            unique(dat_DEMO_fert()$metodo_de_agregacion),
            "<b>Metodología:</b>",
            unique(dat_DEMO_fert()$concepto_estadistico_y_metodologia)
          )
        ))
      })
      
      # Relevancia:
      output$rel_dat_DEMO_fert <- renderUI({
        helpText(HTML(paste(
          "<b>Relvancia:</b>", unique(dat_DEMO_fert()$relevancia)
        )))
      })
      
      
      output$fecha_dat_DEMO_fert <- renderUI({
        if (input$visualizador_DEMO_fert == "Serie de tiempo") {
          tagList(
            ...
            div(
              ...
              sliderTextInput(
                inputId = "fecha_dat_DEMO_fert",
                label = "Seleccione años",
                choices = sort(unique(dat_DEMO_fert()$fecha)),
                selected = c(min(dat_DEMO_fert()$fecha),
                             max(dat_DEMO_fert()$fecha)),
                hide_min_max = T,
                grid = TRUE,
                width = "100%"
              )
      ...
      output$corte_DEMO_fert <- renderUI({
        if (input$indicador_DEMO_fert %in% vars_corte) {
          selectInput(
            inputId = "corte_DEMO_fert",
            label = "Seleccione categorías",
            choices =  dat_DEMO_fert() %>%
              distinct(get(names(dat_DEMO_fert(
              )[, ncol(dat_DEMO_fert())]))) %>%
              pull(),
            selected = dat_DEMO_fert() %>%
              filter(jerarquia == 1) %>%
              distinct(get(names(dat_DEMO_fert(
              )[, ncol(dat_DEMO_fert())]))) %>%
              pull()
      ...
      # Checkbox por pais
      output$sel_DEMO_fert_pais <- renderUI({
        if (input$visualizador_DEMO_fert == "Serie de tiempo") {
          dropdown(
            label = "Seleccione países",
            status = "default",
          ...
          checkboxGroupInput(
            inputId = "chbox_pais_reg_DEMO_fert",
            label = "Mostrar:",
            inline = FALSE,
            choices = c("Países", "Regiones"),
            selected = "Países"
      ...
      dat_DEMO_fert_anual <- reactive({
        if (input$indicador_DEMO_fert %in% vars_corte) {
          dat_DEMO_fert() %>%
            filter(get(names(dat_DEMO_fert()[, ncol(dat_DEMO_fert())])) %in% input$corte_DEMO_fert) %>%
            filter(fecha == input$fecha_DEMO_fert)
      ...
      dat_DEMO_fert_simple <- reactive({
        if (input$indicador_DEMO_fert %in% vars_corte) {
          dat_DEMO_fert() %>%
            filter(get(names(dat_DEMO_fert()[, ncol(dat_DEMO_fert())])) %in% input$corte_DEMO_fert) %>%
            filter(fecha >= input$fecha_dat_DEMO_fert[1] &
                     fecha <= input$fecha_dat_DEMO_fert[2]) %>%
            filter(cod_pais %in% input$chbox_pais_DEMO_fert |
                     pais %in% input$chbox_reg_DEMO_fert)
      ...
      # Gráficos DEMO_fert
      output$p_dat_DEMO_fert <- renderPlot({
        if (input$visualizador_DEMO_fert == "Serie de tiempo") {
          req(input$fecha_dat_DEMO_fert, input$indicador_DEMO_fert)
          
          plot_DEMO_fert <- ggplot(data = dat_DEMO_fert_simple(),
                                    aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
            ...
            labs(x = "",
                 ...
                 caption = wrapit(
                   paste(
                     "Fuente: Unidad de Métodos y Acceso a Datos (FCS - UdelaR) en base a datos de",
                     unique(dat_DEMO_fert_simple()$fuente)
                   )
                 )) +
            scale_y_continuous(labels = addUnits) +
            scale_colour_manual(name = "", values = paleta_expandida) +
            if (input$indicador_DEMO_fert %in% vars_corte) {
              ggtitle(paste0(input$indicador_DEMO_fert, " (", input$corte_DEMO_fert, ")"))
              
            } else {
              ggtitle(paste(input$indicador_DEMO_fert))
              
            }
          
          print(plot_DEMO_fert)
          ggsave(
            "www/indicador DEMO_fert.png",
            width = 30,
            height = 20,
            units = "cm"
          )
          
        } else if (input$visualizador_DEMO_fert == "Anual gráfico") {
          base_plot_DEMO_fert <- dat_DEMO_fert_anual() %>%
            filter(pais_region %in% input$chbox_pais_reg_DEMO_fert)
          
          validate(need(
            nrow(base_plot_DEMO_fert) > 0,
            'No hay datos disponible para esta búsqueda'
          ))
          
          plot_DEMO_fert <- ggplot(base_plot_DEMO_fert,
                                    aes(x = fct_reorder(pais, valor), y = valor)) +
            geom_segment(
              aes(
                x = fct_reorder(pais, valor),
            ...
            labs(x = "",
                 ...
                 caption = wrapit(
                   paste(
                     "Fuente: Unidad de Métodos y Acceso a Datos (FCS - UdelaR) en base a datos de",
                     unique(dat_DEMO_fert_anual()$fuente)
                   )
                 )) +
            scale_y_continuous(labels = addUnits) +
            if (input$indicador_DEMO_fert %in% vars_corte) {
      ...
      output$map_DEMO_fert <- renderLeaflet({
        # Read this shape file with the rgdal library.
        world_spdf <- readOGR(dsn = "data" ,
                              layer = "TM_WORLD_BORDERS_SIMPL-0.3",
                              verbose = FALSE)
        ...
        # Pegar datos
        world_spdf@data <- world_spdf@data %>%
          left_join(select(dat_DEMO_fert_anual(), pais_eng, valor_original),
                    by = c("NAME" = "pais_eng"))
        ...
        leaflet(world_spdf) %>%
          ...
          addLegend(
            ...
            values =  ~ valor_original,
            opacity = 0.9,
            title = wrapit(input$indicador_DEMO_fert),
            position = "bottomleft"
          )
      ...
      # Botón descarga grafico
      output$baja_plot_DEMO_fert <- downloadHandler(filename <- function() {
        paste("indicador DEMO_fert", "png", sep = ".")
      },
      
      content <- function(file) {
        file.copy("www/indicador DEMO_fert.png", file)
      },
      contentType = "www/indicador DEMO_fert")
      
      
      ## Data series temporal
      
      # Data para tabla y exportar
      dat_DEMO_fert_st <- reactive({
        if (input$indicador_DEMO_fert %in% vars_corte) {
          dat_DEMO_fert() %>%
            filter(fecha >= input$fecha_dat_DEMO_fert[1] &
                     fecha <= input$fecha_dat_DEMO_fert[2]) %>%
            filter(cod_pais %in% input$chbox_pais_DEMO_fert |
                     pais %in% input$chbox_reg_DEMO_fert) %>%
            select(pais, fecha, names(dat_DEMO_fert()[, ncol(dat_DEMO_fert())]), valor) %>%
            arrange(desc(fecha), fct_reorder(pais, -valor))
      ...
      # Lista para descarga
      list_dat_DEMO_fert_st <- reactive({
        list_dat_DEMO_fert <- list(
          "Data" = dat_DEMO_fert_st(),
          "Metadata" = dat_DEMO_fert_m(),
          "Data Completa" = dat_DEMO_fert_c()
        )
      })
      ...
      # Lista para descarga
      list_dat_DEMO_fert_a <- reactive({
        list_dat_DEMO_fert <- list(
          "Data" = dat_DEMO_fert_a(),
          "Metadata" = dat_DEMO_fert_m(),
          "Data Completa" = dat_DEMO_fert_c()
        )
      })
      ...
      # Tablas en shiny
      output$tab_dat_DEMO_fert <- renderDT({
        if (input$indicador_DEMO_fert %in% vars_corte) {
          DT::datatable(
            dat_DEMO_fert_st(),
            rownames = FALSE,
            ...
            caption = htmltools::tags$caption(input$indicador_DEMO_fert,
                                              style = "color:black; font-size:110%;")
          ) %>%
            formatCurrency(4, '', mark = ",")
          
        } else if (input$visualizador_DEMO_fert == "Serie de tiempo") {
          DT::datatable(
            dat_DEMO_fert_st(),
            rownames = FALSE,
            colnames = c("País/Región", "Fecha", "Valor"),
        ...
        } else if (input$visualizador_DEMO_fert %in% c("Anual gráfico", "Anual mapa")) {
          DT::datatable(
            dat_DEMO_fert_a(),
            rownames = FALSE,
            colnames = c("País/Región", "Fecha", "Valor"),
      ...
      # Descarga tabla
      output$dl_tabla_dat_DEMO_fert <- downloadHandler(
        filename = function() {
          paste("resultados-", input$indicador_DEMO_fert, ".xlsx", sep = "")
        },
        content = function(file) {
          if (input$visualizador_DEMO_fert == "Serie de tiempo") {
            openxlsx::write.xlsx(list_dat_DEMO_fert_st(), file)
            
          } else if (input$visualizador_DEMO_fert %in% c("Anual gráfico", "Anual mapa")) {
            openxlsx::write.xlsx(list_dat_DEMO_fert_a(), file)
            
          }
        }
      )
      
      ##  23.  DEMO_pob (dat_DEMO_pob)   ====================================
      
      # Data DEMO_pob
      
      dat_DEMO_pob <- reactive({
        req(input$indicador_DEMO_pob)
      ...
      # Titulo
      output$title_dat_DEMO_pob <- renderUI({
        helpText(HTML(unique(dat_DEMO_pob()$nomindicador)))
      })
      
      # Subtitulo
      output$subtitle_dat_DEMO_pob <- renderUI({
        helpText(HTML(unique(dat_DEMO_pob()$definicion)))
      })
      
      # Metodología
      output$info_dat_DEMO_pob <- renderUI({
        helpText(HTML(
          paste(
            "<b>Método de Agregación:</b>",
            unique(dat_DEMO_pob()$metodo_de_agregacion),
            "<b>Metodología:</b>",
            unique(dat_DEMO_pob()$concepto_estadistico_y_metodologia)
          )
        ))
      })
      
      # Relevancia:
      output$rel_dat_DEMO_pob <- renderUI({
        helpText(HTML(paste(
          "<b>Relvancia:</b>", unique(dat_DEMO_pob()$relevancia)
        )))
      })
      
      
      output$fecha_dat_DEMO_pob <- renderUI({
        if (input$visualizador_DEMO_pob == "Serie de tiempo") {
          tagList(
            ...
            div(
              ...
              sliderTextInput(
                inputId = "fecha_dat_DEMO_pob",
                label = "Seleccione años",
                choices = sort(unique(dat_DEMO_pob()$fecha)),
                selected = c(min(dat_DEMO_pob()$fecha),
                             max(dat_DEMO_pob()$fecha)),
                hide_min_max = T,
                grid = TRUE,
                width = "100%"
              )
      ...
      output$corte_DEMO_pob <- renderUI({
        if (input$indicador_DEMO_pob %in% vars_corte) {
          selectInput(
            inputId = "corte_DEMO_pob",
            label = "Seleccione categorías",
            choices =  dat_DEMO_pob() %>%
              distinct(get(names(dat_DEMO_pob(
              )[, ncol(dat_DEMO_pob())]))) %>%
              pull(),
            selected = dat_DEMO_pob() %>%
              filter(jerarquia == 1) %>%
              distinct(get(names(dat_DEMO_pob(
              )[, ncol(dat_DEMO_pob())]))) %>%
              pull()
      ...
      # Checkbox por pais
      output$sel_DEMO_pob_pais <- renderUI({
        if (input$visualizador_DEMO_pob == "Serie de tiempo") {
          dropdown(
            label = "Seleccione países",
            status = "default",
          ...
          checkboxGroupInput(
            inputId = "chbox_pais_reg_DEMO_pob",
            label = "Mostrar:",
            inline = FALSE,
            choices = c("Países", "Regiones"),
            selected = "Países"
      ...
      dat_DEMO_pob_anual <- reactive({
        if (input$indicador_DEMO_pob %in% vars_corte) {
          dat_DEMO_pob() %>%
            filter(get(names(dat_DEMO_pob()[, ncol(dat_DEMO_pob())])) %in% input$corte_DEMO_pob) %>%
            filter(fecha == input$fecha_DEMO_pob)
      ...
      dat_DEMO_pob_simple <- reactive({
        if (input$indicador_DEMO_pob %in% vars_corte) {
          dat_DEMO_pob() %>%
            filter(get(names(dat_DEMO_pob()[, ncol(dat_DEMO_pob())])) %in% input$corte_DEMO_pob) %>%
            filter(fecha >= input$fecha_dat_DEMO_pob[1] &
                     fecha <= input$fecha_dat_DEMO_pob[2]) %>%
            filter(cod_pais %in% input$chbox_pais_DEMO_pob |
                     pais %in% input$chbox_reg_DEMO_pob)
      ...
      # Gráficos DEMO_pob
      output$p_dat_DEMO_pob <- renderPlot({
        if (input$visualizador_DEMO_pob == "Serie de tiempo") {
          req(input$fecha_dat_DEMO_pob, input$indicador_DEMO_pob)
          
          plot_DEMO_pob <- ggplot(data = dat_DEMO_pob_simple(),
                                   aes(x = fecha, y = valor)) +
            geom_line(aes(color = pais), size = 1, alpha = 0.5) +
            geom_point(aes(color = pais), size = 3) +
            theme(axis.text.x = element_text(angle = 0),
            ...
            labs(x = "",
                 ...
                 caption = wrapit(
                   paste(
                     "Fuente: Unidad de Métodos y Acceso a Datos (FCS - UdelaR) en base a datos de",
                     unique(dat_DEMO_pob_simple()$fuente)
                   )
                 )) +
            scale_y_continuous(labels = addUnits) +
            scale_colour_manual(name = "", values = paleta_expandida) +
            if (input$indicador_DEMO_pob %in% vars_corte) {
              ggtitle(paste0(input$indicador_DEMO_pob, " (", input$corte_DEMO_pob, ")"))
              
            } else {
              ggtitle(paste(input$indicador_DEMO_pob))
              
            }
          
          print(plot_DEMO_pob)
          ggsave(
            "www/indicador DEMO_pob.png",
            width = 30,
            height = 20,
            units = "cm"
          )
          
        } else if (input$visualizador_DEMO_pob == "Anual gráfico") {
          base_plot_DEMO_pob <- dat_DEMO_pob_anual() %>%
            filter(pais_region %in% input$chbox_pais_reg_DEMO_pob)
          
          validate(need(
            nrow(base_plot_DEMO_pob) > 0,
            'No hay datos disponible para esta búsqueda'
          ))
          
          plot_DEMO_pob <- ggplot(base_plot_DEMO_pob,
                                   aes(x = fct_reorder(pais, valor), y = valor)) +
            geom_segment(
              aes(
                x = fct_reorder(pais, valor),
            ...
            labs(x = "",
                 ...
                 caption = wrapit(
                   paste(
                     "Fuente: Unidad de Métodos y Acceso a Datos (FCS - UdelaR) en base a datos de",
                     unique(dat_DEMO_pob_anual()$fuente)
                   )
                 )) +
            scale_y_continuous(labels = addUnits) +
            if (input$indicador_DEMO_pob %in% vars_corte) {
      ...
      output$map_DEMO_pob <- renderLeaflet({
        # Read this shape file with the rgdal library.
        world_spdf <- readOGR(dsn = "data" ,
                              layer = "TM_WORLD_BORDERS_SIMPL-0.3",
                              verbose = FALSE)
        ...
        # Pegar datos
        world_spdf@data <- world_spdf@data %>%
          left_join(select(dat_DEMO_pob_anual(), pais_eng, valor_original),
                    by = c("NAME" = "pais_eng"))
        ...
        leaflet(world_spdf) %>%
          ...
          addLegend(
            ...
            values =  ~ valor_original,
            opacity = 0.9,
            title = wrapit(input$indicador_DEMO_pob),
            position = "bottomleft"
          )
      ...
      # Botón descarga grafico
      output$baja_plot_DEMO_pob <- downloadHandler(filename <- function() {
        paste("indicador DEMO_pob", "png", sep = ".")
      },
      
      content <- function(file) {
        file.copy("www/indicador DEMO_pob.png", file)
      },
      contentType = "www/indicador DEMO_pob")
      
      
      ## Data series temporal
      
      # Data para tabla y exportar
      dat_DEMO_pob_st <- reactive({
        if (input$indicador_DEMO_pob %in% vars_corte) {
          dat_DEMO_pob() %>%
            filter(fecha >= input$fecha_dat_DEMO_pob[1] &
                     fecha <= input$fecha_dat_DEMO_pob[2]) %>%
            filter(cod_pais %in% input$chbox_pais_DEMO_pob |
                     pais %in% input$chbox_reg_DEMO_pob) %>%
            select(pais, fecha, names(dat_DEMO_pob()[, ncol(dat_DEMO_pob())]), valor) %>%
            arrange(desc(fecha), fct_reorder(pais, -valor))
      ...
      # Lista para descarga
      list_dat_DEMO_pob_st <- reactive({
        list_dat_DEMO_pob <- list(
          "Data" = dat_DEMO_pob_st(),
          "Metadata" = dat_DEMO_pob_m(),
          "Data Completa" = dat_DEMO_pob_c()
        )
      })
      ...
      # Tablas en shiny
      output$tab_dat_DEMO_pob <- renderDT({
        if (input$indicador_DEMO_pob %in% vars_corte) {
          DT::datatable(
            dat_DEMO_pob_st(),
            rownames = FALSE,
            ...
            caption = htmltools::tags$caption(input$indicador_DEMO_pob,
                                              style = "color:black; font-size:110%;")
          ) %>%
            formatCurrency(4, '', mark = ",")
          
        } else if (input$visualizador_DEMO_pob == "Serie de tiempo") {
          DT::datatable(
            dat_DEMO_pob_st(),
            rownames = FALSE,
            colnames = c("País/Región", "Fecha", "Valor"),
      ...
      # Descarga tabla
      output$dl_tabla_dat_DEMO_pob <- downloadHandler(
        filename = function() {
          paste("resultados-", input$indicador_DEMO_pob, ".xlsx", sep = "")
        },
        content = function(file) {
          if (input$visualizador_DEMO_pob == "Serie de tiempo") {
            openxlsx::write.xlsx(list_dat_DEMO_pob_st(), file)
